...
export class BalanceSheetComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const companyTicker = this._activeRoute.snapshot.params["ticker"];
  ...
  getBalanceSheetByPeriod(companyTicker: string, min: number, max: number) {
    this._apiService
      .getBalanceSheetDataByPeriod(companyTicker, min, max)
      .subscribe({
        next: (res: any) => {
          console.log("res", res);
          this.mockApiResponse = [res.balance_sheet];
          this.financials = this.convertMockApiToFinancialsFullyGeneric(
            this.mockApiResponse
          );
  ...
  //range selection
  selectedRange: RangeValue = { min: 2019, max: 2024 };
